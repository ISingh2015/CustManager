 ...
 * @author Inderjit
 */
public class SalesDetailsTableModel extends AbstractTableModel {
    ...
    public SalesDetailsTableModel(String[] colNames, ArrayList<ElegantBuySellDetails> tableData) {
    ...
    public void setValueAt(Object val, int row, int col) {
        ...
        switch (col) {
            ...
            case 4:
                if (val instanceof String) {
                    ...
                    if (value<=buySell.getInStock()){
                        buySell.setBilledQty(value);
                    } else {
                        done=false;
                    }
                }
                buySell.setUnitAmt((buySell.getBilledQty()* buySell.getBilledRate()) - buySell.getUnitDiscount());
                break;
            case 5:
                if (val instanceof String) buySell.setBilledQty(Double.parseDouble((String) val));
                break;
            case 6:
                if (val instanceof String) {
            ...
            case 7:
                ...
                break;
            case 8:
                if (val instanceof String) buySell.setUnitDiscount(Double.parseDouble((String) val));
                buySell.setUnitAmt((buySell.getBilledQty()* buySell.getBilledRate()) - buySell.getUnitDiscount());
                break;
            case 9:
                if (val instanceof String) {
    ...
    public Object getValueAt(int row, int col) {
        ...
        switch (col) {
            ...
            case 4:
                value = buySell.getBilledQty();
                break;
            case 5:
                value = buySell.getBilledRtnQty();
                break;
            case 6:
                value = buySell.getBilledRate();
                break;
            case 7:
    ...
    public boolean isCellEditable(int row, int col) {
        if (col == 0 || col == 2 || col==3 || col == 5 || col == 9) {
            return false;
        }
        return true;
    }
